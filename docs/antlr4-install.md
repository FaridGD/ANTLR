...
```powershell
.\build\Release\main.exe .\examples\valid\arithmetic.example
```
...
```bash
./build/main ./examples/valid/arithmetic.example
```
...
```bash
# Поток токенов
echo 'x = 3 + 4;' | antlr4-parse grammar/Example.g4 program -tokens

# Дерево разбора (текст)
echo 'x = 3 + 4;' | antlr4-parse grammar/Example.g4 program -tree

# Графическое дерево (нужен GUI)
echo 'x = 3 + 4;' | antlr4-parse grammar/Example.g4 program -gui
```