...
Инструкции по установке всех зависимостей для Windows и Linux — в файле [docs/antlr4-install.md](docs/antlr4-install.md).
...
├── grammar/
│   ├── Example.g4                  # грамматика (чистая, для Visitor/Listener)
...
│   └── main_actions.cpp            # точка входа (подход встроенные действия)
├── examples/
...
## Два подхода к реализации семантики

В проекте представлены **два варианта** реализации семантики для одного и того же языка.

**Вариант 1 — Visitor** (файлы `Example.g4` + `main.cpp` + `ExampleInterpreter.h`). Грамматика описывает только синтаксис. Семантика реализуется в отдельном C++ классе, который наследуется от сгенерированного `ExampleBaseVisitor` и переопределяет `visit`-методы. Это рекомендуемый подход для серьёзных проектов: грамматика остаётся чистым описанием языка, код модульный и тестируемый.

**Вариант 2 — встроенные действия** (файлы `ExampleActions.g4` + `main_actions.cpp`). Семантика встроена прямо в грамматику через конструкции `returns`, `$value`, `{ ... }` — аналог `$$` и `$1` в Bison. Код компактнее, но грамматика привязана к C++ и не может быть переиспользована для другого целевого языка.

Можно также использовать паттерн Listener (автоматический обход дерева с событиями `enter`/`exit`) — он подходит для задач сбора информации.

---

## Задание

Разработать транслятор для языка, описанного в вашем варианте, используя ANTLR4 с целевым языком C++17.

Транслятор должен:

1. Принимать путь к файлу с программой в качестве **аргумента командной строки**.
2. При успешном разборе и выполнении — завершаться с кодом возврата **0**.
...
examples/
├── valid/
│   ├── arithmetic.txt       # арифметические выражения
│   ├── conditionals.txt     # ветвления if/else
│   └── loops.txt            # циклы while
└── invalid/
    ├── missing_semicolon.txt    # ошибка: пропущена точка с запятой
    ├── unknown_token.txt        # ошибка: неизвестный символ
    └── undefined_var.txt        # ошибка: необъявленная переменная
```
...
Первая сборка занимает 3–7 минут: CMake автоматически скачивает ANTLR4 C++ runtime (~100 МБ), JAR-файл ANTLR Tool (~2 МБ), генерирует парсер из `.g4` и компилирует всё. Последующие пересборки — секунды.
...
CMake создаёт два исполняемых файла:

`main` — Visitor-подход (грамматика `Example.g4` + интерпретатор `ExampleInterpreter.h`). Это основной target, который проверяется автотестами.

`main_actions` — встроенные действия (грамматика `ExampleActions.g4`). Демонстрация альтернативного подхода.
...
Положительные примеры (valid/):
  PASS  valid/arithmetic.example

Негативные примеры (invalid/):
  PASS  invalid/missing_semicolon.example

──────────────────────────────────────────
Итог: 2 прошло / 0 упало / 2 всего
```
...
docker compose -f docker/docker-compose.yml run --rm translator run /workspace/examples/valid/arithmetic.example
```
...
3. Запустите пример: `./build/main ./examples/valid/arithmetic.example` — должно вывести `16`, `7`, `26`.
4. Изучите файлы `grammar/Example.g4`, `src/ExampleInterpreter.h` и `src/main.cpp`.
5. Замените грамматику и интерпретатор на свой вариант языка.
6. Добавьте примеры в `examples/valid/` и `examples/invalid/`.
7. Убедитесь, что `./examples/run_tests.sh ./build/main` проходит.
...
```bash
# Дерево разбора
echo '3 + 4 * 5; print x;' | antlr4-parse grammar/Example.g4 program -tree

# Поток токенов
echo '3 + 4 * 5; print x;' | antlr4-parse grammar/Example.g4 program -tokens
```
